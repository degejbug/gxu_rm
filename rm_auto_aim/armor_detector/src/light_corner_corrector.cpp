...
SymmetryAxis LightCornerCorrector::findSymmetryAxis(const cv::Mat &gray_img, const Light &light) {
  ...
  cv::Rect light_box = light.boundingRect();//fyt origin
  ...
  light_box.width = std::min(light_box.width, gray_img.cols - light_box.x -1);
  light_box.height = std::min(light_box.height, gray_img.rows - light_box.y -1);
