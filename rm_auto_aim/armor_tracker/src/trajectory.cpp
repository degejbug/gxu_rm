...
void Trajectory::autoSolveTrajectory(auto_aim_interfaces::msg::Target & target_msg)
{
  ...
  target_msg.position.x = pitch;
  target_msg.position.y = yaw;
    
}

float Trajectory::newtonUpdate(float s, float v, float angle)
{
  float z;
  //t为给定v与angle时的飞行时间
...
float Trajectory::pitchSolve(float s, float z, float v)
{
